...
class Custom_CNNMnist(nn.Module):
    def __init__(self, args, model_config):
        super(CNNMnist, self).__init__()
        self.args = args
        self.model_config = model_config
        self.conv1 = nn.Conv2d(args.num_channels, 10, kernel_size=5)
        self.conv_custom = None
        if self.model_config['num_custom_layers'] > 2:
            self.conv_custom = _make_layers()
    ...
    def _make_layers(self):
        ...
        custom_layers = []
        for i in range(self.model_config['num_custom_layers'] - 2):
            if i == 0:
                custom_layers.append(nn.Conv2d(10, 10, kernel_size=5))
...
class Custom_CNNCifar(nn.Module):
    def __init__(self, args, model_config):
